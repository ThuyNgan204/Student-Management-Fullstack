...
export async function GET(req: Request) {
  ...
  const cohort = searchParams.get("cohort"); // Năm học
  const academicYear = searchParams.get("academic_year"); // ✅ filter theo năm học
  const classSectionId = searchParams.get("class_section_id"); // ✅ filter theo lớp

  if (!studentId) return NextResponse.json({ error: "student_id required" }, { status: 400 });

  // Build filter giống GET /api/grades
  const where: any = { enrollment: { student_id: Number(studentId) } };
  if (academicYear) {
    where.enrollment = {
      ...where.enrollment,
  ...
  if (classSectionId) {
    where.enrollment = {
      ...where.enrollment,
      class_section: { id: Number(classSectionId) },
    };
  }
  ...
  const grades = await prisma.grades.findMany({
    ...
    orderBy: [
      { enrollment: { class_section: { academic_year: "asc" } } },
      { enrollment: { class_section: { semester: "asc" } } },
    ],
  });

  if (!grades.length) return NextResponse.json({ error: "No grades found" }, { status: 404 });
  ...
  const cohortNum = cohort ? Number(cohort) : null;
    ...
    // Tính NĂM X dựa trên cohort và academic_year
    const startYear = parseInt(academicYear.split("-")[0]); // vd: "2023-2024" => 2023
    const yearNumber = startYear - cohortNum + 1; // Năm học thứ X
  ...
  grades.forEach((g) => {
    ...
    weightedScore += (Number(g.total_score) || 0) * credits; // ✅ convert string -> number
  });
  const avgScore = totalCredits ? Number((weightedScore / totalCredits).toFixed(2)) : 0;
  const { letter: avgLetter, status: avgStatus } = calculateLetterGrade(avgScore);
