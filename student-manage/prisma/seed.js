...
async function main() {
  ...
  // Xóa dữ liệu tất cả bảng (đúng thứ tự FK)
  await prisma.grades.deleteMany();
  await prisma.enrollment.deleteMany();
  await prisma.class_section.deleteMany();
  await prisma.major_courses.deleteMany();
  ...
  // Reset lại sequence trong PostgreSQL
  await prisma.$executeRawUnsafe(`
    DO $$ DECLARE
        seq RECORD;
    BEGIN
  ...
  const hashedPassword = await bcrypt.hash("admin123", 10); // 10 rounds
  ...
  console.log("✅ All tables cleared & ID sequences reset!");
}
